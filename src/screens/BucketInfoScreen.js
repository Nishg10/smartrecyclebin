...
const BucketInfoScreen = (props) => {
  ...
  const getData = async () => {
    ...
    const bucketinfo=collection(db,"bucket");
    console.log("bucketList"+bucketinfo);
    const bucketinfosnapshot=await getDocs(bucketinfo);
    const bucketList=bucketinfosnapshot.docs.map(doc=>doc.data());
    console.log(bucketList);
  ...
  const RecommendedCard = ({place}) => {
    ...
    return (
      <ImageBackground style={style.rmCardImage} source={place.category}>
        ...
        <Text
          ...
          <Icon name="delete" size={18} color={place.color} 
          style={{
        ...
        <View
          ...
          <View style={{width: '100%', flexDirection: 'row', marginTop: 5}}>
            ...
            <View style={{flexDirection: 'row'}}>
              ...
              <Text style={{color: COLORS.dark, marginLeft: 5}}>Max Capacity: {place.maxcapacity}</Text>
            </View>
            
            <View style={{flexDirection: 'row'}}>
              <Icon name="battery-alert" size={18} color={COLORS.dark} />
              <Text style={{color: COLORS.dark, fontSize: 13}}>
              Trigger Point: {place.limit}
              </Text>
            </View>
            </View>
          </View>
          <View>
            <View style={{flexDirection: 'row'}}>
                <Icon name="category" size={18} color={COLORS.dark} />
                <Text style={{color: COLORS.dark, fontSize: 13}}>
    ...
    <SafeAreaView style={{flex: 1, backgroundColor: COLORS.white}}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View
          ...
          <Text style={style.headerTitle}>Bucket Information</Text>
...
export default BucketInfoScreen;
